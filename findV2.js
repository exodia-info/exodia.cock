...
let min = 1;
let max = 90;
let topp = Math.floor((max-min)*Math.random() + min)
let left = Math.floor((max-min)*Math.random() + min)
...
function start(){
    ...
    setInterval(posizione, 40000)
}

function posizione(){
    if(exodia.classList == 'exodia nascondi' || container.classList == 'container hide'){
        let exodia = document.getElementById('exodia')
        topp = Math.floor((max-min)*Math.random() + min)
        left = Math.floor((max-min)*Math.random() + min)
        exodia.style.top = `${topp}%`
        exodia.style.left = `${left}%`
        document.getElementById('attenzione').classList.remove('hide')
        setTimeout('attenzione()', 2000)
    }
}

function attenzione(){
    document.getElementById('attenzione').classList.add('hide')
}
